...
static void SteadyOuterX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
      Real time, Real dt, int is, int ie, int js, int je, int ks, int ke, int ngh); //disk hydro outer boundary condition


static void FixRadOuterX1new(MeshBlock *pmb, Coordinates *pco, NRRadiation *pnrrad, 
     const AthenaArray<Real> &w, FaceField &b, AthenaArray<Real> &ir, 
      Real time, Real dt, int is, int ie, int js, int je, int ks, int ke, int ngh); 
      //disk radiation outer boundary condition
...
//                             /(ini_profile(lright,0) - ini_profile(lleft,0));
  ...
  for(int i=is; i<=ie; ++i) {
    ...
    Real temstar = std::max(tem_rmax, tfloor);
    Real rhostar = rho_rmax;   
    ...
    if(x1 > rmax){
      temstar = std::max(tem_rmax, tfloor);
      Real grav_local = grav_rmax * pow(x1/rmax,2.0);
      rhostar = rho_rmax * exp(-grav_local*(x1-rmax)/(temstar));
      rhostar = std::max(rhostar,1.e-8);
      ...
      rhostar = ini_profile(lleft,2) + (x1 - ini_profile(lleft,0)) *
                                  (ini_profile(lright,2) - ini_profile(lleft,2))
                                 /(ini_profile(lright,0) - ini_profile(lleft,0));
      temstar = ini_profile(lleft,1) + (x1 - ini_profile(lleft,0)) *
                                  (ini_profile(lright,1) - ini_profile(lleft,1))
                                 /(ini_profile(lright,0) - ini_profile(lleft,0));
 ...
 //   rho *= (1.0 + amp * ((double)rand()/(double)RAND_MAX-0.5));
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        ...
        Real rhodisk = den0*zfactor*zfactor*zfactor;
        Real rho = std::max(rhodisk, rhostar);
        //disk or star density
        ...
        Real v1, v2, v3;
        VelProfile(x1, x2, x3, rho, v1, v2, v3); //Real v1, v2, v3 = 0.0;
        phydro->u(IM1,k,j,i) = 0;
        phydro->u(IM2,k,j,i) = 0;
        phydro->u(IM3,k,j,i) = 0;

        Real temdisk = p0_over_r0*zfactor;
        Real tem = std::max(temdisk, temstar);
        //disk or star temperature
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=1; i<=ngh; ++i) {
          ...
          //  if (j == je) 
        //printf("midplane tgas: %e\n",tem);
            if (tem < tfloor) {tem = tfloor;}
            prim(IEN,k,j,ie+i) = prim(IDN,k,j,ie+i)*tem;
